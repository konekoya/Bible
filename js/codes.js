...
function allowAny(t, begin1, end1) {
  ...
  for (i = chapter_lines[book_chapters[begin1]]; i < chapter_lines[book_chapters[end1+1]]; i++) {
    ...
    if(searchType == SEARCHANY) { refineElement  = compareElement.substring(0,compareElement.indexOf('|HTTP')); }
    else { refineElement = compareElement.substring(compareElement.indexOf('|HTTP'), compareElement.length); }
